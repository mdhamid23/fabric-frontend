...
const AUTH_SERVER_BASE_URL =
  process.env.AUTH_SERVER_BASE_URL || "http://localhost:9000";

const CLIENT_ID = process.env.OAUTH2_CLIENT_ID || "nextjs-app";
const CLIENT_SECRET = process.env.OAUTH2_CLIENT_SECRET || "changeme";
const REDIRECT_URI =
  process.env.OAUTH2_REDIRECT_URI || "http://localhost:3000/api/auth/callback";
...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    ...
    return NextResponse.redirect(
      new URL(`/api/auth/login`, APP_BASE_URL).toString(),
    );
  }
  ...
  if (
    !storedStateCookie ||
    !storedStateCookie.startsWith(`${returnedState}:`)
  ) {
  ...
  const next = storedStateCookie.split(":").slice(1).join(":") || "/";
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      ...
      console.error(
        "[OAUTH2 CALLBACK] Token exchange failed:",
    ...
    console.log("[SSO Logout] [Next.js CALLBACK] Token response received:", {
      ...
      id_token_preview: tokens.id_token
  ...
  if (tokens.id_token) {
    console.log(
      "[SSO Logout] [Next.js CALLBACK] id_token is present — validating claims...",
    );
    ...
    if (claims["iss"] !== expectedIssuer) {
      console.error(
        "[OAUTH2 CALLBACK] id_token iss mismatch — got:",
        claims["iss"],
        "expected:",
        expectedIssuer,
      );
    ...
    if (!audMatches) {
      console.error(
        "[OAUTH2 CALLBACK] id_token aud mismatch — got:",
        aud,
        "expected:",
        CLIENT_ID,
      );
    ...
    if (typeof claims["exp"] === "number" && claims["exp"] < now) {
      console.error(
        "[OAUTH2 CALLBACK] id_token is expired — exp:",
        claims["exp"],
        "now:",
        now,
      );
    ...
    if (storedNonce && claims["nonce"] !== storedNonce) {
      console.error(
        "[OAUTH2 CALLBACK] id_token nonce mismatch — possible replay attack",
      );
      return NextResponse.redirect(
        new URL(`/api/auth/login`, APP_BASE_URL).toString(),
      );
    ...
    console.warn(
      "[SSO Logout] [Next.js CALLBACK] WARNING: Auth server did NOT return an id_token. Front-channel logout will not work. Check that the 'openid' scope is requested and authorized for nextjs-app.",
    );
  }
  ...
  if (tokens.id_token) {
    ...
    console.log(
      "[SSO Logout] [Next.js CALLBACK] id_token cookie saved (30-day TTL). Logout will use RP-Initiated Logout.",
    );
  } else {
    console.warn(
      "[SSO Logout] [Next.js CALLBACK] id_token cookie NOT saved — logout will fall back to local cookie clear only.",
    );
  }
